        ...
        return this.each(function (index, elem) {
            ...
            function fnCreateGroupRow(sGroupCleaned, sGroup, iColspan) {
                ...
                if (properties.bExpandableGrouping) {
					...
					$(nCell).attr('id', oGroup.dataGroup); //add hook for qa tests
            ...
            function _fnCreateGroup2Row(sGroup2, sGroupLabel, iColspan, oParentGroup) {
                ...
                var oGroup = { id: nGroup2.id, key: sGroup2, text: sGroupLabel, level: oParentGroup.level + 1, groupItemClass: ".group-item-" + dataGroup, dataGroup: dataGroup, aoSubgroups: new Array() };
                ...
                if (properties.bExpandableGrouping && properties.bExpandableGrouping2) {
                    ...
                    if (_fnIsGroupCollapsed(oGroup.dataGroup, 'parent')) {
					...
					$(nCell2).attr('id', oGroup.dataGroup); //add hook for qa tests
            ...
            function _fnGetCleanedGroup(sGroup) {
                ...
                if (sGroup === "" || sGroup == null) {
					...
					try {
						var strRetGroup = sGroup.toString().toLowerCase().replace(/[^a-zA-Z0-9\u0080-\uFFFF]+/g, "-"); //fix for unicode characters (Issue 23)
					} catch (e) { }
				}
            ...
            function fnCollapseGroup(sGroup) {
				...
				if (bUseSecondaryGrouping) {
					...
					$(asExpandedGroups).each(function (index, elem) {
						
						var arr_elem = elem.split("_");
						var index = 0;
						
						if (arr_elem.length == 1 && arr_elem[0] == sGroup) {
							aoGroups[arr_elem[0]].state = "collapsed";
							index = jQuery.inArray(arr_elem[0], asExpandedGroups);
							asExpandedGroups.splice(index, 1);
						} 
						else if (arr_elem.length == 2 && arr_elem[0] == sGroup) {
							aoGroups[elem].state = "collapsed";
							index = jQuery.inArray(elem, asExpandedGroups);
							asExpandedGroups.splice(index, 1);
						}
						else if (arr_elem.length == 2 && elem == sGroup) {
							aoGroups[elem].state = "collapsed";
							index = jQuery.inArray(elem, asExpandedGroups);
							asExpandedGroups.splice(index, 1);
						}
                ...
                if (properties.oHideEffect != null)
					...
					if (bUseSecondaryGrouping) {
						$('tr[class*="group-item-' + sGroup + '"]', oTable).hide();
					} else {
...
//                          sGroupData = oSettings.aoData[oSettings.aiDisplay[iDisplayIndex]]._aData[oSettings.aoColumns[properties.iGroupingColumnIndex].mDataProp];
                        ...
                        if (bUseSecondaryGrouping) {
							...
							if (typeof sGroup2 == 'undefined') {
								sGroup2 = 'unfoldered';
								unfoldered = true;
							}
							else if (sGroup2 == 'zzzzz-unfoldered') {
								unfoldered = true;
							}
                        }
						...
						$(nTrs[i]).attr("parent-group", sGroup);
                        ...
                        if (bUseSecondaryGrouping) {
                            ...
                            if (sLastGroup2 == null || _fnGetCleanedGroup(sGroup2) != _fnGetCleanedGroup(sLastGroup2)) {
                                ...
                                var nGroup2 = oGroup2.nGroup;
							   	...
							   	// sm: don't add group headers for unfoldered items.
								if (!unfoldered) {
									nTrs[i].parentNode.insertBefore(nGroup2, nTrs[i]);
								}
								
								//Default: nTrs[i].parentNode.insertBefore(nGroup2, nTrs[i]);
								
								//sm: hide second group by default 
								if (_fnIsGroupCollapsed(sGroupCleaned)) {
									$(nGroup2).hide();
								}
                                sLastGroup2 = sGroup2;
							...
							// sm: add parent class, so itmes inside unfoldered group are expanded by default
							if (unfoldered) {
								$(nTrs[i]).addClass("group-item-" + sGroupCleaned);
								$(nTrs[i]).addClass('unfoldered-row');
							}
						...
						if (properties.bExpandableGrouping) {
							...
							if (bUseSecondaryGrouping) {
								...
								var secdondary_group = aoGroups[sGroupCleaned].dataGroup + '_' + sGroup2Id;
								
								//keep unfoldered items expanded if level 1 group is expanded.
								if (unfoldered) {
									if (_fnIsGroupCollapsed(sGroupCleaned)) {
										$(nTrs[i]).hide();
									}
								}
								else if (_fnIsGroupCollapsed(secdondary_group) || _fnIsGroupCollapsed(sGroupCleaned)) {
									$(nTrs[i]).hide();
								}
							}
							//default
